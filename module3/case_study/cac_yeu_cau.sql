...
begin
	create table bang_thong_bao (
		thong_bao varchar(50)
	);
	if id_nhan_vien_add in (
		select id_nhan_vien
		from nhan_vien
	)
		then
			if id_khach_hang_add in (
				select id_khach_hang
				from khach_hang
			)
				then
					if id_dich_vu_add in (
						select id_dich_vu
						from dich_vu
					)
						then 
							...
							insert into bang_thong_bao value ('Thêm hợp đồng thành công !');
						else insert into bang_thong_bao value ('ID dịch vụ không hợp lệ !');
					end if;
				else insert into bang_thong_bao value ('ID khách hàng không hợp lệ !');
			end if;
		else insert into bang_thong_bao value ('ID nhân viên không hợp lệ !');
	end if;
    select * from bang_thong_bao;
    drop table if exists bang_thong_bao;
end //
delimiter ;

call Sp_2(111, 1, 1, '2020-09-20', '2020-09-25');
call Sp_2(101, 111, 1, '2020-09-20', '2020-09-25');
call Sp_2(101, 1, 111, '2020-09-20', '2020-09-25');
...
delimiter //
create function func_1(tham_so int)
returns int
deterministic
begin
	declare count int;
		select count(thong_ke.ten_dich_vu) into count
        from thong_ke
        where thong_ke.tong_tien * 23000 > tham_so;
    return count;
end; //
delimiter ;

select func_1(2000000) as 'Số lượng dịch vụ thỏa mãn yêu cầu';
drop function if exists func_1;

-- Câu b :

delimiter //
create function func_2(tham_so int)
returns int
deterministic
begin    
	declare so_ngay int;
        select max(bang_tam.tinh) into so_ngay
        from (
			select datediff(hd.ngay_ket_thuc, hd.ngay_lam_hop_dong) as tinh
			from hop_dong hd
			where hd.id_khach_hang = tham_so
			group by hd.id_hop_dong
        ) as bang_tam;
    return so_ngay;
end; //
...
select func_2(1) as 'Thời gian dài nhất tính theo ngày';
...
--          của các dịch vụ đó (tức là xóa các bảng ghi trong bảng DichVu) và xóa những HopDong sử dụng dịch vụ liên quan (tức là phải xóa những bản ghi trong bảng HopDong)
...
begin
    ...
    where id_loai_dich_vu = 3 and id_dich_vu in (
		select id_dich_vu
        from hop_dong
        where year(ngay_lam_hop_dong) in (2015, 2016, 2017, 2018, 2019)
        group by id_dich_vu
    );
end //
delimiter ;
